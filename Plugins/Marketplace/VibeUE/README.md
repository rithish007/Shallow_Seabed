...
3. *(Optional)* **Edit → Plugins** → enable any of Epic's per-domain toolsets you want alongside
   VibeUE — each below is source-verified to register `AICallable` methods (i.e. it actually adds
   tools, not just a plugin with nothing to call): `GameplayTagsToolset`, `NiagaraToolsets`,
   `MVVMToolset`, `UMGToolSet`, `PCGToolset`, `PhysicsToolsets`, `GASToolsets`, `DataRegistryToolset`,
   `ConfigSettingsToolset`, `ChaosClothAssetToolset`, `GameFeaturesToolset`, `LiveCodingToolset`,
   `SemanticSearchToolset`, `SlateInspectorToolset`, `WorldConditionsToolset`,
   `AutomationTestToolset`, `PluginToolset`, `DataflowAgent`. Skip `StateTreeToolset`,
   `AnimationAssistantToolset`, `ConversationToolset`, `AIModuleToolset`, `SequencerAnimMixerToolset`,
   and `MetaHumanGenerator` — as of UE 5.8 these ship with **zero** `AICallable` methods, so enabling
   them adds nothing (VibeUE's `StateTreeService` etc. already cover those domains in full).
...
**Optional, not auto-enabled (Step 1.3):** Epic's per-domain toolset plugins
(`GameplayTagsToolset`, `NiagaraToolsets`, `MVVMToolset`, `UMGToolSet`, `PCGToolset`,
`PhysicsToolsets`, `GASToolsets`, `DataRegistryToolset`, `ConfigSettingsToolset`,
`ChaosClothAssetToolset`, `GameFeaturesToolset`, `LiveCodingToolset`, `SemanticSearchToolset`,
`SlateInspectorToolset`, `WorldConditionsToolset`, `AutomationTestToolset`, `PluginToolset`,
`DataflowAgent`) each add real `AICallable` tools and are worth enabling if you want that domain's
Epic-native tools too. **Don't bother** enabling `StateTreeToolset`, `AnimationAssistantToolset`,
`ConversationToolset`, `AIModuleToolset`, `SequencerAnimMixerToolset`, or `MetaHumanGenerator` — none
of them register any `AICallable` methods in UE 5.8, so there's nothing for an agent to call.
